      ...
      exec(getDurationCommand(tempAudioFile), (error, audioDuration) => {
        ...
        exec(getDurationCommand(tempVideoFile), (error, videoDuration) => {
          ...
          const ffmpegCommand = `ffmpeg -ss ${convertSecondsToHMS(
            ...
            videostart
          )} -i ${tempVideoFile} -t ${shorterDuration} -vf "scale=720:1280:force_original_aspect_ratio=decrease,pad=720:1280:-1:-1,setsar=1" -c:v libx264 -b:v 500k -c:a aac -strict experimental -preset veryfast ${outputFilePath} `;
