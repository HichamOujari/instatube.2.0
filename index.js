    ...
    .then(() => {
      ...
      exec(getDurationCommand(tempAudioFile), (error, audioDuration) => {
        ...
        exec(getDurationCommand(tempVideoFile), (error, videoDuration) => {
          ...
          const ffmpegCommand = `ffmpeg -i ${tempAudioFile} -i ${tempVideoFile} -t ${reelsDuration} -vf "scale=720:1280:force_original_aspect_ratio=decrease,pad=720:1280:-1:-1,setsar=1" -c:v libx264 -b:v 1M -c:a aac -strict experimental ${outputFilePath}`;
